        ...
        with open(filename, "w") as f:
            for name, d in self.functions.items():
                ...
                if not d:
                    ...
                    line = f"int {name}(const dict &req, int count, int reqid);\n\n"
                f.write(line)
    ...
    def generate_source_process(self):
        ...
        with open(filename, "w") as f:
            for name, d in self.callbacks.items():
                ...
                for field, type_ in d.items():
                    if type_ == "HSIntVolume":
                        args.append("task->task_volume")
                    elif type_ == "HSNum":
    ...
    def generate_source_function(self):
        ...
        with open(filename, "w") as f:
            for name, d in self.functions.items():
                ...
                if req_name in ["reqSecuTransactionSubscribe", "reqSecuTransactionCancel"]:
                    type_ = list(d.values())[1]
                    f.write(
                        f"int {self.class_name}::{req_name}(string type, const dict &req, int count, int reqid)\n")
                    f.write("{\n")
                    ...
                    f.write(f"\tint i = this->api->{name}((char)type, &myreq, count, reqid);\n")
                    f.write("\treturn i;\n")
                    f.write("};\n\n")
                else:
                    f.write(
                        f"int {self.class_name}::{req_name}(const dict &req, int count, int reqid)\n")
                    ...
                    f.write(f"\tint i = this->api->{name}(&myreq, count, reqid);\n")
                    f.write("\treturn i;\n")
                    f.write("};\n\n")
...
if __name__ == "__main__":
    td_generator = ApiGenerator("../include/nsq/HSNsqApi.h", "nsq", "md", "MdApi")
    td_generator.run()
